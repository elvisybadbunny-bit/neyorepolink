...
import { nextTenantId, generateNeyoLoginId, backfillIdPrefixes } from "../src/lib/services/identity.service";
...
async function main() {
  ...
  // Uhuru -> "UA" — both UNCHANGED from what they've always been, since
  // neither one has ever genuinely collided). Skips any tenant that
  // already has idPrefix set, so re-running this seed never re-derives or
  // silently changes an already-stamped prefix.
  const t4Backfill = await backfillIdPrefixes();
  console.log(`✓ Seeded T.4: ID-prefix backfill — ${t4Backfill.backfilled} tenant(s) newly stamped, ${t4Backfill.skipped} already had a real idPrefix.`);

  // A.2.9: a NEYO platform super-admin (belongs to the first tenant for storage,
  // A.2.9 / Y.2: the real NEYO Founder account — unrestricted, everything in
  // the system, per the founder's own explicit instruction ("since am alone
  // for now me the founder can access everything in the system... upgrade
  // it to founder@neyo.co.ke"). Kept on the SAME real neyoLoginId
  // (NEYO-ADMIN-001) so this upgrade is a genuine in-place promotion of the
  // founder's existing account, not a parallel new one — every pre-existing
  // real audit-log row, build-log entry, and NeyoCustomerInterview record
  // this account ever created stays correctly attributed to the same real
  // user id.
  await db.user.upsert({
    where: { neyoLoginId: "NEYO-ADMIN-001" },
    update: { fullName: "NEYO Founder", email: "founder@neyo.co.ke", role: "FOUNDER", passwordHash, isActive: true },
    create: {
      tenantId: tenant.id,
      neyoLoginId: "NEYO-ADMIN-001",
      fullName: "NEYO Founder",
      phone: phone("0700 000 001"),
      email: "founder@neyo.co.ke",
      role: "FOUNDER",
      passwordHash,
    },
  });
  console.log("✓ Seeded NEYO Founder account: founder@neyo.co.ke (FOUNDER — unrestricted, everything in the system)");
  ...
  // --- T.2: a real NEYO-Ops discount campaign demo (NEW_SIGNUPS, one-time
  // first-term discount) — platform-level, not tenant-owned.
  const existingCampaign = await db.discountCampaign.findFirst();
  if (!existingCampaign) {
    const campaignStart = new Date(Date.now() - 5 * 24 * 3600_000);
    const campaignEnd = new Date(Date.now() + 25 * 24 * 3600_000);
    await db.discountCampaign.create({
      data: {
        name: "New School Welcome Offer", appliesTo: "NEW_SIGNUPS", percentOff: 0.2,
        startDate: campaignStart, endDate: campaignEnd,
        createdById: neyoAdmin.id, createdByName: neyoAdmin.fullName,
      },
    });
    console.log("✓ Seeded T.2: real live \"New School Welcome Offer\" campaign (20% off a new school's first term).");
  }

  // --- T.6: a real influencer/promoter code demo — platform-level, not
  // tenant-owned. Deliberately NOT applied to any real seeded tenant (T.2's
  // campaign and T.6's code are mutually exclusive per school), so both
  // real mechanisms are visible side by side without conflicting.
  const existingInfluencerCode = await db.influencerCode.findFirst();
  if (!existingInfluencerCode) {
    await db.influencerCode.create({
      data: {
        code: "NEYO-WANJIKU1", personName: "Wanjiku Njoroge", personPhone: phone("0733 221 100"),
        discountPct: 0.1, commissionKes: 3000,
        createdById: neyoAdmin.id, createdByName: neyoAdmin.fullName,
      },
    });
    console.log("✓ Seeded T.6: real live influencer code NEYO-WANJIKU1 (10% school discount, KES 3,000 commission per school signed).");
  }

  // --- U.1: real KUCCPS degree-programme cluster/course reference data —
  // platform-level, not tenant-owned. Idempotent (match-or-create by real
  // cluster number, matching J.25's non-duplication rule) so it seeds once
  // and can be safely re-run without ever duplicating clusters/courses.
  {
    const { seedKuccpsClusters } = await import("../src/lib/services/pathway-guide.service");
    const neyoAdminActor = {
      id: neyoAdmin.id, tenantId: neyoAdmin.tenantId, neyoLoginId: neyoAdmin.neyoLoginId,
      fullName: neyoAdmin.fullName, phone: neyoAdmin.phone, email: neyoAdmin.email,
      role: neyoAdmin.role as any, secondaryRole: neyoAdmin.secondaryRole as any,
      language: neyoAdmin.language ?? "en",
    };
    const kuccpsResult = await seedKuccpsClusters(neyoAdminActor);
    console.log(`✓ Seeded U.1: real KUCCPS reference data — ${kuccpsResult.clusters} clusters, ${kuccpsResult.coursesCreated} courses created / ${kuccpsResult.coursesUpdated} updated.`);
  }
...
%%EOF`;
  ...
  console.log("✓ Seeded I.27: one saved classroom learning video.");
    ...
    if (!existingBook && librarian) {
      ...
      // T.1 — real per-copy tracking demo: generate real QR codes for a few
      // physical copies of "The River and the Source" so the founder can see
      // per-copy tracking working out of the box (not every book opts in —
      // the other 3 seeded books deliberately stay on plain counts).
      const librarianSessionUser = {
        id: librarian.id, tenantId: tenant.id, neyoLoginId: "", fullName: librarian.fullName,
        phone: librarian.phone, email: librarian.email, role: librarian.role as never,
        secondaryRole: librarian.secondaryRole as never, language: "en",
      };
      const { generateCopiesForBook } = await import("../src/lib/services/library.service");
      // No count given -> defaults to the book's REAL existing copiesTotal
      // (12), so opting in never shrinks the school's real declared copy
      // count — every one of the 12 real physical copies gets its own code.
      const t1Result = await generateCopiesForBook(librarianSessionUser as never, bookIds[0]);
      console.log(`✓ Seeded T.1: ${t1Result.copies.length} real per-copy QR codes generated for "The River and the Source" (demo of per-physical-copy tracking).`);
    }
    ...
    if (!existingMenu) {
      ...
      // T.9 — a real per-level default feeding cost plan (Form 2, Lunch,
      // Term 2), plus turning on real parent-portal self-service requests
      // so the demo shows the whole flow working end to end.
      const existingFeePlan = await db.cafeteriaFeePlan.findFirst({ where: { tenantId: tenant.id } });
      if (!existingFeePlan) {
        await db.cafeteriaFeePlan.create({
          data: {
            tenantId: tenant.id, name: "Form 2 — Lunch — Term 2 2026", level: "Form 2",
            meals: JSON.stringify(["LUNCH"]), termFeeKes: 6500, year: thisYear, term: 2,
          },
        });
        await db.tenant.update({ where: { id: tenant.id }, data: { allowParentCafeteriaRequests: true } });
        console.log("✓ Seeded T.9: real \"Form 2 — Lunch\" fee plan (KES 6,500) + parent cafeteria requests turned on.");
      }
    }
  ...
  // --- T.7: a real whole-school flat fee structure demo (founder-confirmed
  // "option 1" — one flat fee for every class level), sitting side by side
  // with the pre-existing per-level Form 2 structure above, never
  // replacing it. Deliberately Term 1 (not Term 2, already used by the
  // per-level structure above) so both real examples coexist visibly.
  const existingFlatStructure = await db.feeStructure.findFirst({ where: { tenantId: tenant.id, applyToAllLevels: true } });
  if (!existingFlatStructure) {
    await db.feeStructure.create({
      data: {
        tenantId: tenant.id, name: `All levels — Term 1 ${thisYear}`, level: "ALL", applyToAllLevels: true,
        year: thisYear, term: 1,
        items: { create: [{ label: "School development levy", amountKes: 1500 }] },
      },
    });
    console.log("✓ Seeded T.7: real whole-school flat \"School development levy\" structure (KES 1,500, Term 1) — one flat fee for every class level.");
  }

  // --- T.10: real teacher-submitted pending cash entry demo ---
  const existingTeacherCash = await db.teacherCashPaymentRequest.findFirst({ where: { tenantId: tenant.id } });
  if (!existingTeacherCash) {
    await db.tenant.update({ where: { id: tenant.id }, data: { allowTeacherCashPayments: true } });
    const kamauInvoice = await db.invoice.findFirst({ where: { tenantId: tenant.id, invoiceNo: "KHINVSEED2" } });
    const chebet = await db.user.findFirst({ where: { tenantId: tenant.id, email: "f.chebet@karibuhigh.ac.ke" } });
    if (kamauInvoice && chebet) {
      await db.teacherCashPaymentRequest.create({
        data: {
          tenantId: tenant.id, invoiceId: kamauInvoice.id,
          submittedById: chebet.id, submittedByName: chebet.fullName,
          amountKes: 5000, note: "Handed to me at the school gate this morning",
        },
      });
      console.log("✓ Seeded T.10: teacher cash payments turned on + 1 real pending KES 5,000 entry from Chebet Faith awaiting bursar confirmation.");
    }
  }
