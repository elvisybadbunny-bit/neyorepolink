...
CREATE TABLE "IdempotentRequest" (
    ...
    CONSTRAINT "IdempotentRequest_tenantId_fkey" FOREIGN KEY ("tenantId") REFERENCES "Tenant" ("id") ON DELETE CASCADE ON UPDATE CASCADE
);
...
INSERT INTO "new_User" ("canApplyDiscretionaryDecrease", "createdAt", "email", "fullName", "id", "isActive", "language", "lastLoginAt", "lgContrast", "neyoLoginId", "passwordHash", "phone", "popupStyle", "role", "secondaryRole", "shellVersion", "tenantId", "totpEnabled", "totpSecret", "totpVerifiedAt", "updatedAt") SELECT "canApplyDiscretionaryDecrease", "createdAt", "email", "fullName", "id", "isActive", "language", "lastLoginAt", "lgContrast", "neyoLoginId", "passwordHash", "phone", "popupStyle", "role", "secondaryRole", "shellVersion", "tenantId", "totpEnabled", "totpSecret", "totpVerifiedAt", "updatedAt" FROM "User";
DROP TABLE "User";
