...
async function main() {
  ...
  test("exactly 19 roles are defined", () => {
    // Y.2 (2026-07-09): grew from 16 to 19 with the real FOUNDER/NEYO_OPS/
    // NEYO_SUPPORT company role tiers (SUPER_ADMIN kept for backward
  ...
  test("Y.2: FOUNDER can do everything too (real, unrestricted superset)", () => {
    expect(can("FOUNDER", "finance.manage_structure")).toBe(true);
    expect(can("FOUNDER", "user.manage_roles")).toBe(true);
    expect(can("FOUNDER", "neyo.team_manage")).toBe(true);
    expect(can("FOUNDER", "neyo.impersonate")).toBe(true);
  });
  test("Y.2: NEYO_SUPPORT gets real customer-facing access but NOT platform/pricing/team controls", () => {
    expect(can("NEYO_SUPPORT", "neyo.customer_requests")).toBe(true);
    expect(can("NEYO_SUPPORT", "neyo.platform_flags")).toBe(false);
    expect(can("NEYO_SUPPORT", "neyo.pricing_billing")).toBe(false);
    expect(can("NEYO_SUPPORT", "neyo.team_manage")).toBe(false);
    expect(can("NEYO_SUPPORT", "neyo.impersonate")).toBe(false);
  });
  test("Y.2: NEYO_OPS defaults to the SAME safe customer-facing baseline as NEYO_SUPPORT (founder's own 'Founder-only for now' decision)", () => {
    expect(can("NEYO_OPS", "neyo.customer_requests")).toBe(true);
    expect(can("NEYO_OPS", "neyo.platform_flags")).toBe(false);
    expect(can("NEYO_OPS", "neyo.developer_ops")).toBe(false);
    expect(can("NEYO_OPS", "platform.founder_ops")).toBe(false);
  });
  ...
  test("only leadership + super-admin/founder can manage modules", () => {
    // Y.2: FOUNDER is a real, unrestricted superset of SUPER_ADMIN by
    // design (can() returns true for both) — this list now includes it.
    const allowed = ROLES.filter((r) => can(r, "tenant.manage_modules"));
    expect(allowed.sort()).toEqual(
      ["DEPUTY_PRINCIPAL", "FOUNDER", "PRINCIPAL", "SCHOOL_OWNER", "SUPER_ADMIN"].sort()
    );
  });
  test("only leadership + super-admin/founder can export data", () => {
    const allowed = ROLES.filter((r) => can(r, "tenant.export_data"));
    expect(allowed.sort()).toEqual(
      ["FOUNDER", "PRINCIPAL", "SCHOOL_OWNER", "SUPER_ADMIN"].sort()
    );
  });
