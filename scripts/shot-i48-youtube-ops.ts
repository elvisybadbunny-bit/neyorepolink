...
const OUT = "screenshots/i48-youtube-ops.png";
async function main() {
  ...
  await page.evaluate(async () => {
    await fetch("/api/auth/password/login", {
      ...
      body: JSON.stringify({ email: "founder@neyo.co.ke", password: "Karibu2026!" }),
    });
  });
  ...
  await page.waitForSelector("text=YouTube Management & Posting Hub", { timeout: 25000 });
  await page.locator("text=YouTube Management & Posting Hub").scrollIntoViewIfNeeded();
