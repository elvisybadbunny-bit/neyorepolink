...
async function main(){
 const superAdmin=asUser(await db.user.findFirstOrThrow({where:{email:"founder@neyo.co.ke"}}));
 ...
 await page.request.post("http://localhost:3000/api/auth/password/login",{data:{email:"founder@neyo.co.ke",password:"Karibu2026!"}});
