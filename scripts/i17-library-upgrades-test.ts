...
async function main() {
  ...
  try {
    ...
    // Real precision fix (T.1 session): this assertion's own intent is that
    // the ISSUE flow's book/borrower pickers are search-based, not a
    // scrollable dropdown — scoped to JUST the IssueTab function's source so
    // it doesn't false-positive on an unrelated, legitimate <select> added
    // elsewhere in the file (T.1's own copy-status picker, a real 4-option
    // enum control in a completely different component, CopiesDialog).
    const issueTabSrc = client.slice(client.indexOf("function IssueTab"), client.indexOf("\nfunction LoadError"));
    assert(issueTabSrc.includes("Search Book Catalog") && issueTabSrc.includes("Search Borrower") && !issueTabSrc.includes("<select"), "library ISSUE flow's book/borrower pickers are search-only, not dropdown-based");
