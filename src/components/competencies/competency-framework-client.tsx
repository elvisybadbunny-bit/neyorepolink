...
function CompetencyModal({ title, icon: Icon, children, onClose }: { title: string; icon: LucideIcon; children: React.ReactNode; onClose: () => void }) {
  return <div className="fixed inset-0 z-[70] flex items-end justify-center overflow-hidden bg-navy-950/50 p-0 backdrop-blur-md sm:items-center sm:p-4"><Card className="max-h-[calc(100dvh-0.5rem)] w-full max-w-3xl overflow-hidden rounded-t-3xl border-navy-200 bg-white pb-[env(safe-area-inset-bottom)] shadow-2xl dark:border-navy-700 dark:bg-navy-900 sm:max-h-[90dvh] sm:rounded-2xl"><CardHeader className="sticky top-0 z-10 border-b border-navy-100 bg-white backdrop-blur-xl dark:border-navy-800 dark:bg-navy-900"><div className="flex items-center justify-between gap-3"><div className="flex items-center gap-3"><span className="flex h-10 w-10 items-center justify-center rounded-2xl bg-green-50 text-green-700 dark:bg-green-950/40 dark:text-green-200"><Icon className="h-5 w-5" /></span><div><CardTitle>{title}</CardTitle><Badge tone="neutral" className="mt-1">Competency Framework</Badge></div></div><Button variant="ghost" size="sm" onClick={onClose} aria-label="Close">×</Button></div></CardHeader><CardContent className="max-h-[calc(min(92dvh,44rem)-6rem)] overflow-y-auto p-5 sm:p-6">{children}</CardContent></Card></div>;
}
