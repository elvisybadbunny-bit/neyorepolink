...
import { MonitorPlay, Video, VideoOff, Mic, MicOff, PhoneOff, Users, ScreenShare, Shield, Hand, MessageCircle, Check, Maximize2, PictureInPicture2 } from "lucide-react";
import { Button } from "@/components/ui/button";
import { useToast } from "@/components/ui/toast";
import { NeyoLogo } from "@/components/brand/neyo-logo";
import { useVideoPictureInPicture } from "@/lib/hooks/use-video-picture-in-picture";
...
export function OnlineClassRoomClient({ roomId }: { roomId: string }) {
  ...
  // T.5b — a real, standard video PiP toggle so a user can keep watching the
  // live class stage in a small floating window while browsing elsewhere.
  const videoPip = useVideoPictureInPicture(localVideoRef);
