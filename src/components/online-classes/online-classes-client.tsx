...
export function OnlineClassesClient() {
  ...
  return <div className="grid gap-4 lg:grid-cols-3">
    ...
    <Card className={data.canManage ? "lg:col-span-2" : "lg:col-span-3"}>
      ...
      <CardContent className="space-y-3">
        ...
        {data.sessions.map((s:any)=><div key={s.id} className="rounded-2xl border border-navy-100 bg-white p-3 dark:border-navy-800 dark:bg-navy-900">
          ...
          {s.canControl && <div className="mt-3 flex gap-2"><Button size="sm" disabled={saving || s.status === "RUNNING"} onClick={()=>act({action:"running", id:s.id}, "Online class running in this class") }><Play className="h-4 w-4" /> Start</Button><Button size="sm" variant="secondary" disabled={saving || s.status !== "RUNNING"} onClick={()=>act({action:"ended", id:s.id}, "Online class ended") }><Square className="h-4 w-4" /> End</Button></div>}
        </div>)}
