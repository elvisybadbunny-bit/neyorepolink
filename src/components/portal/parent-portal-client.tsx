...
import { CafeteriaRequestCard } from "./cafeteria-request-card";
import { FamilySplitPaymentCard } from "./family-split-payment-card";
...
interface ChildDetail {
  ...
  child: { id: string; name: string; admissionNo: string; photoUrl: string | null; className: string | null; classId: string | null; hasActiveMealCard?: boolean };
...
function ChildView({ id, onBack, isCurriculumEngineEnabled = false }: { id: string; onBack: () => void; isCurriculumEngineEnabled?: boolean }) {
  ...
  return (
    <div className="space-y-4">
      ...
      {/* Y.1 NEYO Pathway Guide (career + subject-selection questionnaire) */}
      <ParentPathwayGuideCard studentId={id} studentName={data.child.name} />
      ...
      {/* T.9 — real cafeteria enrollment/cancel requests */}
      <CafeteriaRequestCard studentId={data.child.id} studentName={data.child.name} hasActiveCard={Boolean(data.child.hasActiveMealCard)} />
