...
// ---- T.10 — real, pending cash-payment entries (school confirms) ----------

interface CashRequestRow { id: string; invoiceNo: string; amountKes: number; status: string; createdAt: string; decidedAt: string | null; rejectReason: string | null }
interface OpenInvoiceOpt { id: string; invoiceNo: string; description: string; balanceKes: number; dueDate: string }
interface StudentHit { id: string; name: string; admissionNo: string }

function CashPaymentsTab() {
  const { toast } = useToast();
  const [requests, setRequests] = React.useState<CashRequestRow[] | null>(null);
  const [submitting, setSubmitting] = React.useState(false);

  const load = React.useCallback(async () => {
    const res = await fetch("/api/teacher/cash-payments");
    const json = await res.json();
  ...
  return (
    <div className="space-y-3">
      <div className="flex items-center justify-between">
        <p className="text-xs text-navy-400">Record cash you've genuinely received — it stays pending until the school office confirms the money reached them.</p>
        <Button onClick={() => setSubmitting(true)}><Plus className="h-4 w-4" /> Record cash received</Button>
      </div>
      {requests.length === 0 ? (
        <EmptyState icon={Banknote} title="No cash entries yet" description="Record real cash you've received from a parent — the school confirms it before it counts as paid." action={<Button onClick={() => setSubmitting(true)}><Plus className="h-4 w-4" /> Record cash received</Button>} />
      ) : (
        <div className="space-y-2">
          {requests.map((r) => (
            <Card key={r.id}>
              <CardContent className="flex flex-wrap items-center justify-between gap-2 p-4">
                <div>
                  <p className="text-sm font-semibold text-navy-900 dark:text-navy-50">KES {r.amountKes.toLocaleString("en-KE")} — {r.invoiceNo}</p>
                  {r.status === "REJECTED" && r.rejectReason && <p className="text-xs text-red-600 dark:text-red-400">{r.rejectReason}</p>}
                </div>
                <Badge tone={r.status === "CONFIRMED" ? "green" : r.status === "REJECTED" ? "red" : "amber"}>{r.status.toLowerCase()}</Badge>
              </CardContent>
            </Card>
          ))}
        </div>
      )}
      {submitting && <CashPaymentDialog onClose={() => setSubmitting(false)} onDone={() => { setSubmitting(false); toast({ title: "Sent to the office for confirmation", tone: "success" }); load(); }} />}
    </div>
  );
}

function CashPaymentDialog({ onClose, onDone }: { onClose: () => void; onDone: () => void }) {
  const { toast } = useToast();
  const [q, setQ] = React.useState("");
  const [hits, setHits] = React.useState<StudentHit[]>([]);
  const [student, setStudent] = React.useState<StudentHit | null>(null);
  const [invoices, setInvoices] = React.useState<OpenInvoiceOpt[] | null>(null);
  ...
  React.useEffect(() => {
    ...
    const t = setTimeout(async () => {
      const res = await fetch(`/api/teacher/cash-payments/lookup?q=${encodeURIComponent(q)}`);
      const json = await res.json();
      if (json.ok) setHits(json.data.students);
    }, 250);
  ...
  async function pickStudent(h: StudentHit) {
    ...
    const res = await fetch("/api/teacher/cash-payments/lookup", {
      method: "POST", headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ studentId: h.id }),
    });
    const json = await res.json();
    if (json.ok) {
      setInvoices(json.data.invoices);
      if (json.data.invoices[0]) { setInvoiceId(json.data.invoices[0].id); setAmount(String(json.data.invoices[0].balanceKes)); }
    }
  }
  ...
  async function submit() {
    ...
    try {
      const res = await fetch("/api/teacher/cash-payments", {
        method: "POST", headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ invoiceId, amountKes: Number(amount), note: note || undefined }),
      });
      const json = await res.json();
      if (json.ok) onDone();
    ...
    <div className="fixed inset-0 z-50 flex items-end justify-center overflow-hidden bg-navy-950/50 p-0 backdrop-blur-sm sm:items-center sm:p-4" onClick={onClose}>
      <div className="max-h-[calc(100dvh-0.5rem)] w-full max-w-md overflow-y-auto overscroll-contain rounded-t-3xl border border-navy-200 bg-white p-4 pb-[max(1rem,env(safe-area-inset-bottom))] shadow-pop dark:border-navy-700 dark:bg-navy-900 sm:max-h-[90dvh] sm:rounded-3xl sm:p-6" onClick={(e) => e.stopPropagation()}>
        <div className="mb-4 flex items-center justify-between">
          <h3 className="text-base font-bold text-navy-900 dark:text-navy-50">Record cash received</h3>
        ...
        <div className="space-y-3">
          {student ? (
            <p className="flex items-center justify-between rounded-xl bg-warm-50 px-3 py-2 text-sm dark:bg-navy-800">
              <span className="font-medium">{student.name} — {student.admissionNo}</span>
              <button onClick={() => { setStudent(null); setInvoices(null); }} className="text-xs text-navy-400 underline">change</button>
            </p>
          ) : (
            <div className="relative">
              <Label>Find the student (your own class)</Label>
              ...
              {hits.length > 0 && (
                <div className="absolute top-full left-0 z-10 mt-1 max-h-40 w-full overflow-y-auto rounded-2xl border border-navy-100 bg-white p-1.5 shadow-pop dark:border-navy-800 dark:bg-navy-950">
                  {hits.map((h) => (
                    <button key={h.id} onClick={() => pickStudent(h)} className="flex w-full items-center justify-between rounded-xl px-3 py-2 text-left text-xs hover:bg-navy-50 dark:hover:bg-navy-800">
                      <span className="font-semibold text-navy-800 dark:text-navy-100">{h.name}</span>
                      <span className="font-mono text-[10px] text-navy-400">{h.admissionNo}</span>
                    </button>
                  ))}
                </div>
              )}
          ...
          {invoices && invoices.length > 0 && (
            <div>
              <Label>Invoice</Label>
              <select value={invoiceId} onChange={(e) => { setInvoiceId(e.target.value); const inv = invoices.find((i) => i.id === e.target.value); if (inv) setAmount(String(inv.balanceKes)); }} className="mt-1 w-full rounded-xl border border-navy-200 bg-white px-3 py-2 text-sm dark:border-navy-700 dark:bg-navy-900">
                {invoices.map((i) => <option key={i.id} value={i.id}>{i.invoiceNo} — balance KES {i.balanceKes.toLocaleString("en-KE")}</option>)}
              </select>
            </div>
          )}
          {invoices && invoices.length === 0 && <p className="text-xs text-amber-600">This student has no open fee balance right now.</p>}

          <div><Label>Amount received (KES)</Label><Input type="number" value={amount} onChange={(e) => setAmount(e.target.value)} /></div>
          <div><Label>Note (optional)</Label><Input value={note} onChange={(e) => setNote(e.target.value)} placeholder="e.g. Handed to the bursar at 2pm" /></div>
          <Button onClick={submit} disabled={saving || !invoiceId || !amount} className="w-full">
            {saving ? <Loader2 className="h-4 w-4 animate-spin" /> : <Banknote className="h-4 w-4" />} Send for confirmation
          </Button>
        </div>
      </div>
    </div>
