...
export function GateClient({ canManage, canPanic, canIssuePass, canApprovePass, currentUserId, canScanAttendance = false, canScanPayment = false, canCheckGatePass = true }: { canManage: boolean; canPanic: boolean; canIssuePass: boolean; canApprovePass: boolean; currentUserId: string; canScanAttendance?: boolean; canScanPayment?: boolean; canCheckGatePass?: boolean }) {
  ...
  async function checkPass() {
    // Z.1 — the actual gate EXIT check is a real, live security decision
    // (should this student be allowed to leave right now), never a record
    // to simply save for later — unlike issuing a pass, this is
    // deliberately NEVER queued offline, so a guard is never falsely told
    // a pass is verified when nobody actually checked it against the
    // school's real, live records.
    if (!online) {
      toast({ title: "Cannot verify — no connection", description: "Use your own judgement or call the office. This check will NOT be marked as done until you're back online.", tone: "error" });
      return;
    }
    const res = await fetch("/api/security", {
      method: "POST", headers: { "Content-Type": "application/json" },
...
function PassDialog({ students, onClose, onDone }: { students: StudentOpt[]; onClose: () => void; onDone: () => void }) {
  ...
  async function save() {
    setSaving(true);
    try {
      // Z.1 — real offline-safe queueing: proposing/issuing a gate pass
      // while the office/gate has no connection now genuinely saves and
      // syncs later (the server-side idempotency key prevents a real
      // duplicate pass being minted on replay).
      const result = await queuedPost(
        "/api/security",
        { action: "gatePass", ...f, returnBy: f.returnBy || undefined, escortName: f.escortName || undefined },
        `Gate pass · ${students.find((s) => s.id === f.studentId)?.name ?? "student"}`
      );
      if (result.queued) {
        toast({ title: "Saved offline — will issue the real pass once you're back online", tone: "success" });
        onDone();
      } else if (result.ok) {
        toast({ title: "Pass request submitted", tone: "success" });
        onDone();
      } else {
        toast({ title: "Failed — check the details and try again", tone: "error" });
      }
