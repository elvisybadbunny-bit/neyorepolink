...
export function ImportWizard() {
  ...
  // instead of holding the wizard's spinner hostage until every row is
  ...
  async function commit() {
    ...
    try {
      ...
      if (runInBackground && json.data.runningInBackground) {
        toast({ title: `Importing ${preview.rows.length} rows in the background`, description: "Keep working elsewhere — NEYO will tell you the moment it's done (see the tasks badge, top right).", tone: "info" });
        setStep(1);
        return;
      }
