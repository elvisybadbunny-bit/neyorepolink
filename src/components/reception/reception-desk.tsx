...
function useSaver() {
  ...
  /**
   * Z.1 — real offline-safe save: the front desk keeps working without a
   * connection (a real edge case for Kenyan schools during an internet
   * shutdown). Uses the SAME real offline queue as Attendance/Gate Pass —
   * `queuedPost()` sends a real Idempotency-Key so a later sync never
   * mints a duplicate visitor badge / payment record.
   */
  async function runQueued(url: string, body: unknown, label: string, onQueued: () => void, onDone: (data: any) => void, okMsg: string) {
    setSaving(true);
    try {
      const result = await queuedPost(url, body, label);
      if (result.queued) {
        ...
        toast({ title: "Could not save — check the details and try again", tone: "error" });
      }
    } finally { setSaving(false); }
  }
  return { saving, run, runQueued };
}

// ---- visitor dialog --------------------------------------------------------
function VisitorDialog({ onClose, onSaved }: { onClose: () => void; onSaved: (v: Visitor | null) => void }) {
  const { saving, runQueued } = useSaver();
  const [f, setF] = React.useState({ name: "", phone: "", idNumber: "", purpose: "", host: "" });
  const set = (k: string, v: string) => setF((p) => ({ ...p, [k]: v }));

  // Z.1 — real offline-safe sign-in: while offline, the front desk still
  // records the visitor locally and syncs (with a real badge number) the
  // moment the connection returns — the server-side idempotency key on
  // this exact action prevents a real duplicate badge being minted.
  function save() {
    void runQueued(
      "/api/reception/visitors",
      f,
      `Visitor sign-in · ${f.name || "visitor"}`,
      () => onSaved(null),
      (d) => onSaved({ id: d.id, name: f.name, phone: f.phone || null, purpose: f.purpose, host: f.host || null, badgeNo: d.badgeNo, signedInAt: new Date().toISOString(), signedOutAt: null }),
      "Signed in"
    );
  }
...
function PaymentDialog({ onClose, onSaved }: { onClose: () => void; onSaved: () => void }) {
  const { saving, run, runQueued } = useSaver();
  ...
  function submit() {
    ...
    // biometric verification required by this school) can be recorded
    // while offline and synced later — the server-side idempotency key
    // prevents a real duplicate payment record for the same handover of
    // money. A biometric-gated payment NEVER queues offline (fingerprint/
    // Face ID verification genuinely requires a live connection to the
    // server to issue and consume the real single-use ticket).
    const doSave = (biometricTicket: string | null) =>
      biometricTicket
        ? run("/api/reception/payments", { ...f, amount: f.amount, biometricTicket }, () => onSaved(), "Payment recorded")
        : runQueued("/api/reception/payments", { ...f, amount: f.amount }, `Cash payment · KES ${f.amount || "0"}`, () => onSaved(), () => onSaved(), "Payment recorded");
