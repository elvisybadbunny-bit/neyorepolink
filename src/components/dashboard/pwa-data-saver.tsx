...
/**
 * I.84 Bundle Saver + Z.1 real offline-first extension. Founder's own
 * confirmed decision this session ("auto_on_login"): every school gets a
 * background best-effort save of their own core data the first time they
 * log in each day, no toggle needed — but the school/staff member can still
 * turn it off. The real, server-side `User.bundleSaverEnabled` field (not
 * just a local browser flag) is now the source of truth, so the preference
 * follows the same person across devices — matching the exact pattern
 * already used for popup-style/lg-contrast. `localStorage` is kept only as
...
const LOCAL_KEY = "neyo-bundle-saver-enabled";
...
export function PwaDataSaverCard() {
  ...
  React.useEffect(() => {
    (async () => {
      const hasCache = await loadCachedSummary();

      // Real server-side preference is the source of truth. `localStorage`
      // is only a fast first-paint cache to avoid a flash of the wrong
      // toggle state while the real fetch is in flight.
      const localCached = localStorage.getItem(LOCAL_KEY);
      if (localCached !== null) setEnabled(localCached === "true");

      try {
        const res = await fetch("/api/me/bundle-saver", { cache: "no-store" });
        const json = await res.json();
        if (json.ok) {
          const realEnabled = Boolean(json.data.enabled);
          setEnabled(realEnabled);
          localStorage.setItem(LOCAL_KEY, String(realEnabled));

          // Founder's own confirmed "auto_on_login" behaviour: if the real
          // preference is on and this device has no saved snapshot yet,
          // do a real, best-effort background sync automatically — no
          // toggle needed, and it never blocks or interrupts the page.
          if (realEnabled && !hasCache) {
            void handleSync(true);
          }
        }
      } catch {
        // Never block the dashboard on this — the card just shows the last
        // known local state until the next successful check.
      } finally {
        setReady(true);
      }
    })();
  ...
  async function handleSync(silent = false) {
    ...
    try {
      ...
      if (!silent) toast({ title: e instanceof Error ? e.message : "Could not sync offline bundle.", tone: "error" });
    } finally {
      setSyncing(false);
    }
  }
  ...
  async function handleToggle(val: boolean) {
    ...
    try {
      await fetch("/api/me/bundle-saver", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ enabled: val }),
      });
    } catch {
      // The local toggle already reflects the choice; a later page load
      // will simply re-sync with the server once connectivity returns.
    }
    if (val) {
      await handleSync();
    } else {
    ...
    <Card className="h-full flex flex-col justify-between">
      <CardHeader>
        ...
        <p className="text-xs text-navy-400">
          On by default: NEYO saves key school data on this device for this app only, so it's still visible during a real internet outage. You can turn this off any time.
        </p>
      </CardHeader>
      <CardContent className="space-y-4 flex-1 flex flex-col justify-between">
        ...
        <div className="flex gap-2">
          <Button size="sm" variant="secondary" disabled={syncing || !enabled} onClick={() => handleSync(false)} className="h-10 text-xs font-bold flex-1">
