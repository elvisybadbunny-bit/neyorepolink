...
import { Phone, PhoneOff, Mic, MicOff, Sparkles, HelpCircle, UserCheck, X, Loader2, PictureInPicture2 } from "lucide-react";
import { Card, CardContent, CardHeader, CardTitle } from "@/components/ui/card";
import { Button } from "@/components/ui/button";
import { Badge } from "@/components/ui/badge";
import { useToast } from "@/components/ui/toast";
import { useDocumentPictureInPicture } from "@/lib/hooks/use-document-picture-in-picture";
...
export function DashboardIntercomClient() {
  ...
  // T.5b — Picture-in-Picture: keep an ACTIVE call visible in a small
  // floating window while the user browses other NEYO pages or a
  // different browser tab entirely. Only offered once genuinely connected
  // (never for the idle directory view — nothing worth floating yet).
  const pip = useDocumentPictureInPicture();
  const pipContentRef = React.useRef<HTMLDivElement>(null);
    ...
    <Card className="h-full flex flex-col justify-between">
      <CardHeader>
        <CardTitle className="flex items-center justify-between text-base">
          ...
          {connected && pip.supported && (
            <button
              onClick={() => {
                if (pip.active) { pip.close(); return; }
                if (pipContentRef.current) void pip.open(pipContentRef.current, { width: 320, height: 300, title: "NEYO Intercom — active call" });
              }}
              title={pip.active ? "Bring call back to this page" : "Pop out this call"}
              className="flex h-7 w-7 items-center justify-center rounded-full text-navy-400 hover:bg-navy-100 dark:hover:bg-navy-800"
            >
      ...
      <CardContent className="space-y-4 flex-1 flex flex-col justify-between">
        {!activeCall ? (
          ...
          <div ref={pipContentRef} className="rounded-2xl border border-green-200 bg-green-50/20 p-5 text-center space-y-4 animate-fade-in flex-1 flex flex-col justify-center">
            <div>
