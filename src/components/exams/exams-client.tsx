...
function MarksEntry({ examId, maxMarks, subjects, classes, onSaved }: {
  ...
  const doSave = React.useCallback(async (rows: SheetStudent[]) => {
    ...
    setSaving(true);
    try {
      // Z.1 — real offline-safe autosave: exam marks are entered on a real
      // upsert-per-student-per-subject basis (naturally safe to replay,
      // same shape as Attendance's own register) — a teacher who loses
      // connection mid-marking keeps typing, and the real queued marks
      // sync automatically the moment the connection returns.
      const result = await queuedPost(
        "/api/exams/marks",
        { examId, subjectId, classId, marks: rows.map((s) => ({ studentId: s.id, marks: s.marks })) },
        `Exam marks · ${classId}`
      );
      if (result.queued) {
        dirtyRef.current = false;
        toast({ title: "Saved offline — will sync when you're back online", tone: "success" });
        setSavedAt(new Date().toLocaleTimeString("en-KE", { hour: "2-digit", minute: "2-digit", second: "2-digit" }) + " (offline)");
      } else if (result.ok) {
        dirtyRef.current = false;
        setSavedAt(new Date().toLocaleTimeString("en-KE", { hour: "2-digit", minute: "2-digit", second: "2-digit" }));
        onSaved();
      } else {
        toast({ title: "Autosave failed", tone: "error" });
      }
