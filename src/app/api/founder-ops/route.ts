...
export async function GET(req: NextRequest) {
  try {
    ...
    if (view === "settings") {
      ...
      const neyoStaff = await db.user.findMany({ where: { role: { in: ["FOUNDER", "SUPER_ADMIN", "NEYO_OPS", "NEYO_SUPPORT"] }, isActive: true }, select: { id: true, fullName: true, email: true, phone: true, role: true, createdAt: true } });
  ...
  try {
    ...
    switch (body.action) {
      ...
      case "update_school_subscription": {
        ...
        // U.2 — real, honest cancelledAt stamp, set exactly once on the
        // real transition INTO CANCELLED (never overwritten on subsequent
        // saves, never approximated via `updatedAt`).
        const existingSub = await db.subscription.findUnique({ where: { tenantId: inputSub.tenantId }, select: { status: true, cancelledAt: true } });
        const enteringCancelled = inputSub.status === "CANCELLED" && existingSub?.status !== "CANCELLED";
        const leavingCancelled = inputSub.status !== "CANCELLED" && existingSub?.status === "CANCELLED";
        ...
        const updatedSub = await db.subscription.upsert({
          ...
          create: {
            ...
            graceEndsAt,
            cancelledAt: inputSub.status === "CANCELLED" ? new Date() : null,
          },
          update: {
            planKey: inputSub.planKey,
            status: inputSub.status,
            grandfatheredPrice: inputSub.grandfatheredPrice,
            graceEndsAt,
            ...(enteringCancelled ? { cancelledAt: new Date() } : {}),
            ...(leavingCancelled ? { cancelledAt: null } : {}),
          },
        });
