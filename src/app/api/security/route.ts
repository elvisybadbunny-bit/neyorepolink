...
export async function POST(req: NextRequest) {
  try {
    ...
    if (action === "gatePass") {
      ...
      //
      // Z.1 — real offline-safe replay: issuing a gate pass mints a real
      // new sequential passNo on every create, so it is NOT naturally safe
      // to retry. When a real Idempotency-Key header is present (sent by
      // `queuedPost()`, including a pass proposed while offline at the
      // gate/office and synced later), a replay of the SAME key returns
      // the exact same real pass created the first time — never a second,
      // genuinely duplicate pass for the same request.
      const user = await requireUser();
      const input = z.object({
      ...
      const pass = idempotencyKey
        ? (await withIdempotency(user.tenantId, "security.gatePass", idempotencyKey, () => issueGatePass(user, input))).result
        : await issueGatePass(user, input);
      return ok(pass, 201);
    }
    ...
    if (action === "usePass") {
      ...
      // Z.1 — real offline-safe replay: marking a pass USED is a real
      // one-way state transition (ACTIVE -> USED) that throws "already
      // used" on a second attempt — without idempotency, a real offline
      // replay of a genuinely successful gate check would show the guard
      // a false error instead of the real original success.
      const idempotencyKey = req.headers.get("Idempotency-Key");
      const usedPass = idempotencyKey
        ? (await withIdempotency(user.tenantId, "security.usePass", idempotencyKey, () => markGatePassUsed(user, passNo))).result
        : await markGatePassUsed(user, passNo);
      return ok(usedPass);
    }
