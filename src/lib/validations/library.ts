...
// T.1 — an optional copyId lets the caller pin the issue to one specific
// physical copy (from a scan or a picker); omitted entirely for a book that
// hasn't opted into per-copy tracking, which keeps behaving exactly as
// before (count-based availability only).
...
// T.1 — per-physical-copy tracking.
export const generateCopiesSchema = z.object({
  bookId: z.string().min(1),
  /** How many NEW copy codes to generate — defaults to filling up to copiesTotal. */
  count: z.coerce.number().int().min(1).max(1000).optional(),
});
export type GenerateCopiesInput = z.infer<typeof generateCopiesSchema>;

export const COPY_STATUSES = ["AVAILABLE", "OUT", "LOST", "DAMAGED", "RETIRED"] as const;
