...
async function recordSmsMargin(tenantId: string) {
  ...
  // U.1 — real, school-CHOSEN SMS spend budget alert check (best-effort,
  // never blocks a send). Lazily imported to avoid a circular dependency
  // (cost-cockpit.service.ts -> notification.service.ts -> this file).
  try {
    const { checkSmsSpendAlert } = await import("@/lib/services/cost-cockpit.service");
    await checkSmsSpendAlert(tenantId);
  } catch {
    // Budget-alert checking must never block or fail an actual SMS send.
  }
}
