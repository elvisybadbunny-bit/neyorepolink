...
export async function createStudent(
  ...
  return withTenant(user.tenantId, async () => {
    ...
    if (input.classId && currentTerm) {
      ...
      if (cls) {
        // T.7 — real fallback order: (1) an exact class override, (2) a
        // real per-level structure, (3) the real whole-school flat "ALL"
        // structure — so a genuinely flat-fee school's newly enrolled
        // student is STILL auto-invoiced correctly, in every real class
        // dimension, without needing a per-level structure to exist at all.
        const { ALL_LEVELS_SENTINEL } = await import("@/lib/services/finance.service");
        const structure =
          ...
          await tenantDb().feeStructure.findFirst({ where: { level: ALL_LEVELS_SENTINEL, classId: null, applyToAllLevels: true, year: currentTerm.year, term: currentTerm.term }, include: { items: true } } as never);
        if (structure) {
    ...
    if (input.classId !== undefined && input.classId !== before.classId) {
      ...
      if (term) {
        ...
        if (input.classId) {
          ...
          if (newClass) {
            // T.7 — same real 3-step fallback as new-enrollment auto-invoicing.
            const { ALL_LEVELS_SENTINEL } = await import("@/lib/services/finance.service");
            const structure =
              ...
              await tenantDb().feeStructure.findFirst({ where: { level: ALL_LEVELS_SENTINEL, classId: null, applyToAllLevels: true, year: term.year, term: term.term }, include: { items: true } } as never);
            if (structure) {
              const total = structure.items.reduce((sum, item) => sum + item.amountKes, 0);
              const invoiceNo = await nextTenantId(user.tenantId, "INVOICE");
              await db.invoice.create({
