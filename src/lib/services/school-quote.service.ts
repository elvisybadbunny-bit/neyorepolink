...
export async function createQuoteRequest(input: CreateQuoteRequestInput) {
  ...
  // Best-effort: notify NEYO's real company accounts (FOUNDER/legacy
  // SUPER_ADMIN, plus any real NEYO_OPS/NEYO_SUPPORT staff who handle
  // these — Y.2) a new (possibly formal) quote request came in, reusing the
  // real, existing notification cascade — never a new notification system.
  try {
    const { notify } = await import("@/lib/services/notification.service");
    const opsUsers = await db.user.findMany({ where: { role: { in: ["FOUNDER", "SUPER_ADMIN", "NEYO_OPS", "NEYO_SUPPORT"] }, isActive: true }, select: { id: true, tenantId: true } });
