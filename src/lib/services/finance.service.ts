...
// T.7 — the real sentinel level value a whole-school flat structure is
// stored under. Never a real class level a school would type themselves
// ("ALL" cannot collide with "Form 2"/"Grade 4"/etc.), so it's a safe,
// unambiguous marker for batchInvoice()'s and every fee-lookup's fallback.
// Exported so every other real fee-lookup site (e.g. student.service.ts's
// auto-invoice-on-enrollment/on-class-change) falls back to the SAME real
// whole-school structure, never a second competing "ALL" convention.
export const ALL_LEVELS_SENTINEL = "ALL";
...
export async function listStructures(user: SessionUser) {
  return withTenant(user.tenantId, async () => {
    ...
    return rows.map((s) => ({
      id: s.id, name: s.name, level: s.level, classId: s.classId,
      applyToAllLevels: s.applyToAllLevels, year: s.year, term: s.term,
...
/**
 * T.7 — founder-confirmed (2026-07-08): "option 1" (one real flat structure
 * for the whole school) PLUS real per-class-level fee differences must still
 * reflect correctly on every real student dimension. `applyToAllLevels`
 * covers the flat case (stored as the real `ALL` sentinel level, classId
 * forced null — a whole-school rate and an exact-class override are
 * mutually exclusive by definition); the pre-existing `level`/`classId`
 * override keeps covering the "different classes pay different amounts"
 * case exactly as before — neither replaces the other.
 */
export async function createStructure(user: SessionUser, input: { level: string; classId?: string; applyToAllLevels?: boolean; year: number; term: number; items: { label: string; amountKes: number }[] }) {
  return withTenant(user.tenantId, async () => {
    const level = input.applyToAllLevels ? ALL_LEVELS_SENTINEL : input.level;
    const classId = input.applyToAllLevels ? null : (input.classId ?? null);

    const dup = await tenantDb().feeStructure.findFirst({ where: { level, classId, year: input.year, term: input.term } });
    if (dup) {
      throw new FinanceError(
        "DUPLICATE",
        input.applyToAllLevels
          ? `A whole-school flat fee structure for Term ${input.term} ${input.year} already exists. Edit it instead.`
          : `A fee structure for ${input.level}${input.classId ? " (this exact class)" : ""}, Term ${input.term} ${input.year} already exists.`
      );
    }
    ...
    const name = input.applyToAllLevels
      ? `All levels — Term ${input.term} ${input.year}`
      : `${classLabel ?? input.level} — Term ${input.term} ${input.year}`;
    const s = await tenantDb().feeStructure.create({
      data: {
        name, level, classId, applyToAllLevels: Boolean(input.applyToAllLevels),
        year: input.year, term: input.term,
        items: { create: input.items },
      } as never,
    });
    await audit(user, "finance.structure_created", s.id, { level, applyToAllLevels: Boolean(input.applyToAllLevels), term: input.term, year: input.year });
    return s;
  });
}
...
export async function batchInvoice(user: SessionUser, structureId: string, dueDate: string) {
  return withTenant(user.tenantId, async () => {
    ...
    // T.7 — a real ALL_LEVELS_SENTINEL ("ALL") structure matches EVERY
    // real active class regardless of level, so a school with a genuinely
    // flat fee invoices the whole school in one action, not once per level.
    const classes = await tenantDb().schoolClass.findMany({
      where: structure.applyToAllLevels
        ? { archived: false }
        : structure.classId
          ? { id: structure.classId, archived: false }
          : { level: structure.level, archived: false },
    });
    ...
    if (students.length === 0) throw new FinanceError("EMPTY", structure.applyToAllLevels ? "No active students found in the whole school." : `No active students in ${structure.level}.`);
...
// ---------------------------------------------------------------------------
// T.11a — real, school-configurable fee-reminder cadence
// ---------------------------------------------------------------------------

export async function feeReminderSchedule(user: SessionUser) {
  return withTenant(user.tenantId, async () => {
    const tenant = await db.tenant.findUniqueOrThrow({
      where: { id: user.tenantId },
      select: { feeReminderGraceDays: true, feeReminderDedupeDays: true },
    });
    return tenant;
  });
}

export async function setFeeReminderSchedule(user: SessionUser, input: { feeReminderGraceDays: number; feeReminderDedupeDays: number }) {
  return withTenant(user.tenantId, async () => {
    const graceDays = Math.max(0, Math.min(90, Math.trunc(input.feeReminderGraceDays)));
    const dedupeDays = Math.max(1, Math.min(90, Math.trunc(input.feeReminderDedupeDays)));
    const row = await db.tenant.update({
      where: { id: user.tenantId },
      data: { feeReminderGraceDays: graceDays, feeReminderDedupeDays: dedupeDays },
      select: { feeReminderGraceDays: true, feeReminderDedupeDays: true },
    });
    await audit(user, "finance.reminder_schedule_updated", user.tenantId, row);
    return row;
  });
}
 ...
 * of every overdue invoice — quota-checked, deduped per the school's own
 * real, configurable cadence (T.11a: Tenant.feeReminderGraceDays/
 * feeReminderDedupeDays, replacing the old fixed 3-day-after-due-date /
 * repeat-every-3-days rule — both default to the exact values the fixed
 * rule already used, so no school's cadence silently changes).
 */
export async function sendFeeReminders(tenantId: string) {
  const tenant = await db.tenant.findUniqueOrThrow({
    where: { id: tenantId },
    select: { name: true, feeReminderGraceDays: true, feeReminderDedupeDays: true },
  });
  const graceDays = tenant.feeReminderGraceDays;
  const dedupeDays = tenant.feeReminderDedupeDays;

  const now = Date.now() + 3 * 3600_000; // Nairobi "now"
  const cutoffDate = new Date(now - graceDays * 24 * 3600_000).toISOString().slice(0, 10);
  const dedupeCutoff = new Date(now - dedupeDays * 24 * 3600_000);
  const overdue = await db.invoice.findMany({
    where: {
      tenantId,
      status: { in: ["UNPAID", "PARTIAL"] },
      dueDate: { lt: cutoffDate },
      OR: [{ reminderSentAt: null }, { reminderSentAt: { lt: dedupeCutoff } }],
    },
    take: 100,
  });
