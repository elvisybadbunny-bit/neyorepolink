...
import { AuthError, effectivePermissionsForUser, type SessionUser } from "@/lib/core/session";
import { isFounderTier, isNeyoCompanyRole, isRole } from "@/lib/core/roles";

/**
 * Begin impersonating a target user from an admin's session.
 * Y.2: FOUNDER/legacy SUPER_ADMIN always allowed. A NEYO_OPS/NEYO_SUPPORT
 * account is allowed ONLY with the real, individually-granted
 * "neyo.impersonate" permission (defense-in-depth — the API route already
 * checks this too, but this service is the real, final gate).
 */
export async function startImpersonation(
  ...
  if (!isFounderTier(admin.role)) {
    const effective = await effectivePermissionsForUser(admin as unknown as SessionUser);
  ...
  if (isRole(target.role) && isNeyoCompanyRole(target.role)) {
