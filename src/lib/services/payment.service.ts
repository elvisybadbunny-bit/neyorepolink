...
export async function handleCallback(
  ...
  if (parsed.status === "PAID") {
    // T.14 — a real multi-child split plan (recorded at STK-push time)
    // takes priority: if one exists for this exact real payment, apply it
    // directly (one real allocation per child's own real invoice) and skip
    // BOTH the generic single-invoice accountRef-matching below AND the
    // generic single-invoice `onPaymentPaid()` hook — a split payment was
    // never meant to land on just one invoice.
    const { applyFamilyPaymentSplit } = await import("@/lib/services/family.service");
    const wasSplitPayment = await applyFamilyPaymentSplit(payment.id).catch(() => false);
