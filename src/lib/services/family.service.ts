...
// ---------------------------------------------------------------------------
// T.14 — Multi-Child Payment Splitting
// ---------------------------------------------------------------------------

export class FamilyPaymentSplitError extends Error {
  constructor(public code: "NOT_FOUND" | "INVALID", message: string) {
    super(message);
    this.name = "FamilyPaymentSplitError";
  }
}

/**
 * A real parent-initiated payment across some/all of their linked
 * children's real open invoices. Reuses the EXISTING, already-battle-
 * tested `initiateStkPush()` as the actual STK mechanism (never a new
 * parallel payment system) — this function's only real job is deciding
 * HOW MUCH of the one real incoming amount goes to which child's real
 * open invoice, recording that real plan BEFORE the callback lands, and
 * letting the real M-Pesa callback (via `applyFamilyPaymentSplit()`)
 * apply it once the money genuinely arrives.
 *
 * Founder-confirmed (2026-07-08) rounding rule: on SPLIT_EQUALLY, if the
 * total doesn't divide evenly into whole KES, the 1-2 KES remainder goes
 * to whichever child currently has the LARGEST real remaining balance —
 * and the split NEVER produces decimal KES amounts (every child's share
 * is a real whole integer, computed with integer division + remainder
 * distribution, exactly matching how M-Pesa itself only ever moves whole
 * shillings).
 */
export async function splitFamilyPayment(
  user: SessionUser,
  input: { strategy: "ONE_CHILD" | "SPLIT_EQUALLY"; amountKes: number; studentIds: string[]; phone: string }
) {
  return withTenant(user.tenantId, async () => {
    const scope = await scopeWhere(user);
    const students = await tenantDb().student.findMany({
      where: { AND: [{ id: { in: input.studentIds } }, scope] },
    });
    if (students.length !== input.studentIds.length) {
      throw new FamilyPaymentSplitError("NOT_FOUND", "One or more children were not found (or are not yours).");
    }

    // Real open invoice per target child (the oldest still-open one — the
    // exact same "which invoice does a one-tap action apply to" rule
    // `familyForStudent()`'s own `openInvoiceId` already uses).
    const allocations: { studentId: string; studentName: string; invoiceId: string; balanceKes: number }[] = [];
    for (const st of students) {
      const openInvoice = await tenantDb().invoice.findFirst({
        where: { studentId: st.id, status: { in: ["UNPAID", "PARTIAL"] } },
        orderBy: { dueDate: "asc" },
      });
      if (!openInvoice) {
        throw new FamilyPaymentSplitError("INVALID", `${fullName(st)} has no real open invoice to pay.`);
      }
      const balance = balanceOf(openInvoice);
      allocations.push({ studentId: st.id, studentName: fullName(st), invoiceId: openInvoice.id, balanceKes: balance });
    }

    let items: { studentId: string; studentName: string; invoiceId: string; amountKes: number }[];

    if (input.strategy === "ONE_CHILD") {
      const target = allocations[0];
      if (input.amountKes > target.balanceKes) {
        throw new FamilyPaymentSplitError("INVALID", `Amount exceeds ${target.studentName}'s real remaining balance of KES ${target.balanceKes.toLocaleString("en-KE")}.`);
      }
      items = [{ studentId: target.studentId, studentName: target.studentName, invoiceId: target.invoiceId, amountKes: input.amountKes }];
    } else {
      // SPLIT_EQUALLY — real integer division, remainder to the child with
      // the largest real remaining balance (founder-confirmed rule), never
      // a decimal KES amount on any child's share.
      const n = allocations.length;
      const base = Math.floor(input.amountKes / n);
      const remainder = input.amountKes - base * n;
      const totalOpenBalance = allocations.reduce((s, a) => s + a.balanceKes, 0);
      if (input.amountKes > totalOpenBalance) {
        throw new FamilyPaymentSplitError("INVALID", `Amount exceeds the family's combined real open balance of KES ${totalOpenBalance.toLocaleString("en-KE")}.`);
      }
      const largestBalanceIdx = allocations.reduce((maxIdx, a, i, arr) => (a.balanceKes > arr[maxIdx].balanceKes ? i : maxIdx), 0);
      items = allocations.map((a, i) => ({
        studentId: a.studentId, studentName: a.studentName, invoiceId: a.invoiceId,
        amountKes: base + (i === largestBalanceIdx ? remainder : 0),
      }));
      // Real, honest guard: a share must never exceed that specific child's
      // own real open balance (a genuinely small family combined balance
      // spread thin could otherwise overshoot one child's own invoice).
      for (const [i, item] of items.entries()) {
        if (item.amountKes > allocations[i].balanceKes) {
          throw new FamilyPaymentSplitError("INVALID", `An equal split of KES ${input.amountKes.toLocaleString("en-KE")} would overpay ${item.studentName}'s invoice. Choose a smaller amount or pay children individually.`);
        }
      }
    }

    // Real STK push for the exact real total — reuses the existing,
    // already-battle-tested payment.service.ts mechanism, never a new one.
    ...
    const result = await initiateStkPush(user.tenantId, {
      ...
      phone: input.phone,
      accountRef: input.strategy === "ONE_CHILD" ? students[0].admissionNo : `FAMILY-SPLIT-${students.length}`,
      description: input.strategy === "ONE_CHILD" ? `Fees — ${items[0].studentName}` : `Fees — split across ${items.length} children`,
    });

    // Real plan, recorded BEFORE the callback lands, so the callback knows
    // exactly how to allocate the one real incoming amount.
    const plan = await db.familyPaymentSplitPlan.create({
      data: {
        tenantId: user.tenantId, paymentId: result.paymentId, initiatedById: user.id,
        strategy: input.strategy, totalAmountKes: input.amountKes,
        items: { create: items },
      },
      include: { items: true },
    });
    ...
    await db.auditLog.create({
      data: {
        ...
        action: "finance.family_payment_split_initiated", entityType: "familyPaymentSplitPlan", entityId: plan.id,
        metadata: JSON.stringify({ strategy: input.strategy, totalAmountKes: input.amountKes, items }),
      },
    });

    return { paymentId: result.paymentId, checkoutRequestId: result.checkoutRequestId, plan };
  });
}

/**
 * Callback hook: once the real STK payment behind a split plan lands PAID,
 * apply each real allocated share to its OWN real invoice — reusing the
 * exact real per-invoice ledger update `onPaymentPaid()` already uses
 * (paidKes/status), just applied once per real child instead of once per
 * payment. Idempotent — a plan already APPLIED is never re-applied.
 */
export async function applyFamilyPaymentSplit(paymentId: string) {
  const plan = await db.familyPaymentSplitPlan.findUnique({ where: { paymentId }, include: { items: true } });
  if (!plan || plan.status === "APPLIED") return false;

  for (const item of plan.items) {
    const inv = await db.invoice.findUnique({ where: { id: item.invoiceId } });
    if (!inv) continue;
    const paid = inv.paidKes + item.amountKes;
    const status = paid >= inv.totalKes - inv.discountKes ? "PAID" : paid > 0 ? "PARTIAL" : "UNPAID";
    await db.invoice.update({ where: { id: inv.id }, data: { paidKes: paid, status } });
    await db.auditLog.create({
      data: {
        tenantId: plan.tenantId, actorName: "M-Pesa (family split)",
        action: "finance.invoice_paid_mpesa_split", entityType: "invoice", entityId: inv.id,
        metadata: JSON.stringify({ amountKes: item.amountKes, studentName: item.studentName, splitPlanId: plan.id }),
      },
    });
  }

  await db.familyPaymentSplitPlan.update({ where: { id: plan.id }, data: { status: "APPLIED", appliedAt: new Date() } });
  return true;
}
