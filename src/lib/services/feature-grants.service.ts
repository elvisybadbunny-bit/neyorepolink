 ...
 *
 * T.3 — a custom, one-off feature built specifically for a school it never
 * existed as a curated `REVENUE_FEATURE_KEYS` entry (that list is a small,
 * pre-defined catalog of NEYO's OWN premium modules, not an open-ended
 * registry for bespoke per-school builds). `allowCustomKey: true` is used
 * ONLY by `custom-feature-request.service.ts`'s real delivery step, so a
 * genuinely new custom feature key can be granted without requiring it to
 * first be added to the curated revenue-feature catalog — the exact same
 * real grant storage/audit path either way, never a second mechanism.
 */
export async function setFeatureGrant(
  ...
  if (!allowCustomKey && !REVENUE_FEATURE_KEYS.includes(featureKey)) {
  ...
  try {
    await db.auditLog.create({
      data: {
        ...
        metadata: JSON.stringify({ featureKey, targetSchool: tenant.name, note: note ?? null, custom: allowCustomKey }),
      },
    });
  } catch {}
