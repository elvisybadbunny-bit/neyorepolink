  ...
  return withTenant(user.tenantId, async () => {
    ...
    return {
      child: {
        ...
        // T.9 — a real, live signal for the portal's cafeteria enrollment
        // card, so it can offer "request to cancel" vs "request to enroll"
        // without a second round-trip.
        hasActiveMealCard: await tenantDb().mealCard.findFirst({ where: { studentId: child.id, active: true }, select: { id: true } }).then(Boolean),
      },
      attendance,
