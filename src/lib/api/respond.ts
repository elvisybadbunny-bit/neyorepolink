...
import { TimetableGovernanceError } from "@/lib/services/timetable-governance.service";
...
import { TeacherCashPaymentError } from "@/lib/services/teacher-cash-payment.service";
import { ParentNudgeError } from "@/lib/services/parent-nudge.service";
import { CustomFeatureRequestError } from "@/lib/services/custom-feature-request.service";
import { BackgroundJobError } from "@/lib/services/background-job.service";
import { DiscountCampaignError } from "@/lib/services/discount-campaign.service";
import { InfluencerCodeError } from "@/lib/services/influencer-code.service";
import { FamilyPaymentSplitError } from "@/lib/services/family.service";
...
import { PathwayGuideError } from "@/lib/services/pathway-guide.service";
import { NeyoTeamError } from "@/lib/services/neyo-team.service";
import { NeyoSupportError } from "@/lib/services/neyo-support.service";
import { FounderDashboardError } from "@/lib/services/founder-dashboard.service";
import { UnitEconomicsError } from "@/lib/services/unit-economics.service";
import { CostCockpitError } from "@/lib/services/cost-cockpit.service";
import { VenueError } from "@/lib/services/venue.service";
...
export function handleError(err: unknown) {
  ...
  if (err instanceof TimetableGovernanceError) {
    const status = err.code === "NOT_FOUND" ? 404 : err.code === "FORBIDDEN" ? 403 : err.code === "CONFLICT" ? 409 : 422;
    return fail(err.code, err.message, status);
  }
  ...
  // T.10 — teacher pending cash payments.
  if (err instanceof TeacherCashPaymentError) {
    const status =
      err.code === "NOT_FOUND" ? 404 :
      err.code === "FORBIDDEN" ? 403 :
      err.code === "ALREADY" ? 409 : 422;
  ...
  // T.11b — "never logged in" parent nudge.
  if (err instanceof ParentNudgeError) {
    const status = err.code === "QUOTA" ? 402 : 422;
    return fail(err.code, err.message, status);
  }

  // T.3 — school-requested custom feature pipeline.
  if (err instanceof CustomFeatureRequestError) {
    const status = err.code === "NOT_FOUND" ? 404 : err.code === "STATE" ? 409 : 422;
    return fail(err.code, err.message, status);
  }

  // T.5a — background task runner.
  if (err instanceof BackgroundJobError) {
    const status = err.code === "NOT_FOUND" ? 404 : err.code === "BUSY" ? 409 : 422;
    return fail(err.code, err.message, status);
  }

  // T.2 — NEYO-Ops discount campaigns.
  if (err instanceof DiscountCampaignError) {
    const status = err.code === "NOT_FOUND" ? 404 : err.code === "ALREADY" ? 409 : 422;
    return fail(err.code, err.message, status);
  }
  ...
  if (err instanceof FamilyPaymentSplitError) {
    const status = err.code === "NOT_FOUND" ? 404 : err.code === "INVALID" ? 422 : 422;
    return fail(err.code, err.message, status);
  }
  ...
  // Y.1 NEYO Pathway Guide.
  if (err instanceof PathwayGuideError) {
    const status =
      err.code === "NOT_FOUND" ? 404 :
      err.code === "FORBIDDEN" ? 403 :
      err.code === "PAYMENT_REQUIRED" ? 402 : 422;
    return fail(err.code, err.message, status);
  }

  // Y.2 NEYO Support Console.
  if (err instanceof NeyoSupportError) {
    return fail(err.code, err.message, 403);
  }

  // Y.2 NEYO Team & Access.
  if (err instanceof NeyoTeamError) {
    const status =
      err.code === "NOT_FOUND" ? 404 :
      err.code === "FORBIDDEN" ? 403 :
      err.code === "DUPLICATE" ? 409 : 422;
  ...
  // U.3 Founder Morning Dashboard / Ask Bundi / Product Analytics / Compliance.
  if (err instanceof FounderDashboardError) {
    const status =
      err.code === "NOT_FOUND" ? 404 :
      err.code === "FORBIDDEN" ? 403 : 422;
    return fail(err.code, err.message, status);
  ...
  // U.2 Unit Economics Dashboard.
  if (err instanceof UnitEconomicsError) {
    const status = err.code === "NOT_FOUND" ? 404 : 422;
    return fail(err.code, err.message, status);
  }

  // U.1 NEYO Ops Real Company Cost Cockpit.
  if (err instanceof CostCockpitError) {
    const status = err.code === "NOT_FOUND" ? 404 : err.code === "FORBIDDEN" ? 403 : 422;
    return fail(err.code, err.message, status);
  }

  // Z.3 Real Venue/Lab system.
