...
export const PERMISSIONS = [
  ...
  // Y.2 — real, fine-grained NEYO company permission tiers (replacing the
  // single undifferentiated "SUPER_ADMIN sees everything" model). FOUNDER
  // gets all of these implicitly (same as SUPER_ADMIN); NEYO_OPS/
  // NEYO_SUPPORT get a real, deliberately narrower subset by default, with
  // individual accounts able to be granted extra ones via `NeyoTeamMember`.
  "neyo.customer_requests", // quotes, demo requests, custom feature requests, onboarding/inquiries — the real customer-facing queue
  "neyo.metrics_view", // read-only company metrics/build log/cadence dashboard
  "neyo.platform_flags", // pause/release modules & features platform-wide — sensitive, NEYO_OPS-eligible
  "neyo.pricing_billing", // pricing engine, discount campaigns, influencer codes, revenue grants, subscription/billing overrides — Founder-sensitive
  "neyo.developer_ops", // developer center, API partner keys, storage intelligence, curriculum library — NEYO_OPS-eligible internal tooling
  "neyo.impersonate", // impersonate a school's login for support debugging (A.2.9) — Founder-sensitive
  "neyo.team_manage", // create/suspend/edit NEYO company accounts — Founder-only
  // Calendar (A.17)
  "calendar.view",
  "calendar.manage",
...
export const ROLE_PERMISSIONS: Record<Role, Permission[]> = {
  SUPER_ADMIN: [...PERMISSIONS], // legacy — everything (kept for backward compatibility)
  FOUNDER: [...PERMISSIONS], // real NEYO founder — everything, unrestricted, per the founder's own explicit instruction ("since am alone for now me the founder can access everything in the system")
  // Y.2 — NEYO_OPS: broader internal company tooling (developer center,
  // storage intelligence, platform flags, curriculum library, metrics,
  // customer requests) but explicitly EXCLUDES the most sensitive
  // founder-only actions: pricing/billing/discount/revenue-grant changes,
  // impersonating a school's login, and managing NEYO team accounts.
  // Y.2 — NEYO_OPS: founder chose "keep everything else Founder-only for
  // now (safer default)" — so a fresh NEYO_OPS account starts with the SAME
  // real customer-facing baseline as NEYO_SUPPORT (never platform flags,
  // pricing, developer tooling, or the full Founder Ops console by
  // default). The distinction from NEYO_SUPPORT today is only the ROLE
  // LABEL/intent; real extra access (platform flags, developer center,
  // etc.) is granted per-account, one at a time, via `NeyoTeamMember`, as
  // the founder actually brings on real team members and decides what each
  // one specifically needs — never assumed upfront.
  NEYO_OPS: [
    "neyo.customer_requests",
    "neyo.metrics_view",
    "audit.view",
    "reports.view",
    "calendar.view",
  ],
  // Y.2 — NEYO_SUPPORT: customer-facing only, per the founder's own words
  // ("answer inquiries, check requests, demo requests, quotation requests
  // and any other thing a customer may need — onboarding, planning and
  // guidance") — never platform flags, pricing, developer tooling, or team
  // management. A specific NEYO_SUPPORT account can be granted MORE via
  // `NeyoTeamMember.extraPermissionsJson` on a real, individual basis.
  NEYO_SUPPORT: [
    "neyo.customer_requests",
    "neyo.metrics_view",
    "calendar.view",
