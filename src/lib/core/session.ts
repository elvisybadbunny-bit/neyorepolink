...
import { can, permissionsForRole, PERMISSIONS, type Permission } from "@/lib/core/permissions";
...
export async function effectivePermissionsForUser(user: SessionUser): Promise<Permission[]> {
  ...
  // Y.2 — a real NEYO_OPS/NEYO_SUPPORT account may have been individually
  // granted extra permissions beyond its base role (the founder's own
  // "pick permissions individually per account" answer). FOUNDER/
  // SUPER_ADMIN already have every permission via permissionsForRole()
  // above, so this lookup only ever WIDENS a narrower company-staff role,
  // never narrows anything.
  if (user.role === "NEYO_OPS" || user.role === "NEYO_SUPPORT") {
    const teamMember = await db.neyoTeamMember.findUnique({ where: { userId: user.id }, select: { extraPermissionsJson: true, active: true } });
    if (teamMember && !teamMember.active) return []; // a suspended NEYO account has zero effective permissions, even though the User row itself isn't deleted
    if (teamMember?.extraPermissionsJson) {
      let extra: string[] = [];
      try { extra = JSON.parse(teamMember.extraPermissionsJson); } catch { extra = []; }
      const validExtra = extra.filter((p): p is Permission => (PERMISSIONS as readonly string[]).includes(p));
      return mergeUnique([...base, ...validExtra]);
    }
    return base;
  }
...
/**
 * Y.2 — real NEYO founder identity: whenever a check allows `SUPER_ADMIN`,
 * `FOUNDER` also satisfies it (FOUNDER is a strict superset by design — the
 * founder's own explicit instruction is "I can access everything in the
 * system"). This single central rule means the ~50 pre-existing
 * `requireRole("SUPER_ADMIN")` call sites across the whole codebase don't
 * each need editing to also list "FOUNDER" — a genuinely safer, less
 * error-prone approach than a 50-file find/replace that could easily miss
 * one and silently lock the founder out of a real feature.
 */
