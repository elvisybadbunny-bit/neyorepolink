...
## Print isolation and ID-card spacing repair

Prepared browser-print regions now carry `data-print-document`. During printing, NEYO hides the surrounding shell, page headings, tabs and unrelated cards and positions only that document at the page origin. Class lists, exam performance rosters and timetable print packs use this rule. Batch ID-card PDF layout no longer relies on React-PDF's unsupported/inconsistent `gap`; it applies explicit right/bottom margins per calculated row and column while preserving the school-set width and height.
