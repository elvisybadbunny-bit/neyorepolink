...
**Status**: DD.2 and DD.12 (the confirmed bugs) are next up for a real root-cause investigation + fix, per the CHUNK process (real DB, real regression test, tsc-clean, committed the moment it's stable).
