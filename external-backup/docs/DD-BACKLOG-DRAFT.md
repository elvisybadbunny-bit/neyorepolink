...
Still open: the founder's own further request for cross-grade/whole-school grouping beyond single-grade ("if the whole school got the same method it renders whole school" and "if a group has a certain same eg grade 1-3 it renders that to avoid repeating") has NOT yet been designed or built — needs its own `ask_user` clarification pass (e.g. auto-detected agreement across contiguous grades vs. a school-defined named group) before being built.

Next: clarify and build DD.10's cross-grade/whole-school grouping, then DD.13 (double-period cell merging) and DD.14 (TeacherSubject dropdown audit).
