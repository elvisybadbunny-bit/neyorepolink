...
**DD.10 — PARTIALLY DONE.** The "Schedule Rules" dialog (`ClassConfigModal`, i.e. the time-slot/period configuration) still needs the same whole-grade-grouping UI treatment applied — the backend (`getTimetableConfigAgreementForLevel`/`saveTimetableConfigForLevel`) already exists and is tested (part of the same `66e814f` commit), only the UI wiring for this specific dialog remains. Separately, the founder's own words also ask for a further step beyond single-grade grouping: "if the whole school got the same method it renders whole school" and "if a group has a certain same eg grade 1-3 it renders that to avoid repeating" — i.e. when MULTIPLE grades (or the whole school) share identical settings, the UI should collapse them into one shared view too, not just one card per grade. This cross-grade/whole-school grouping has not yet been designed and will need its own `ask_user` clarification pass (e.g. auto-detected agreement across contiguous grades vs. a school-defined named group) before being built.

Next: finish DD.10 (Schedule Rules dialog whole-grade wiring, then the cross-grade/whole-school grouping design), then DD.13 (double-period cell merging) and DD.14 (TeacherSubject dropdown audit).
